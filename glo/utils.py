...
# from AutoAugment.autoaugment2 import AutoAugment
# from create_dataset_cifar100 import CompatibleDataset
from glo.model import weights_init
...
from glo import vgg_metric, resnet
...
NAGParams = collections.namedtuple('NAGParams', 'nz force_l2 is_pixel z_init is_classifier disc_net loss data_name noise_proj shot')
...
def format_im(ims_gen, mu, sd):
	if ims_gen.size(1) == 3:
		rev_idx = maybe_cuda(torch.LongTensor([2, 1, 0]))
	elif ims_gen.size(1) == 1:
		rev_idx = maybe_cuda(torch.LongTensor([0]))
	else:
		arr = [i for i in range(ims_gen.size(1))]
		rev_idx = maybe_cuda(torch.LongTensor(arr))
	# Generated images
...
def get_loader_with_idx(dataset, batch_size, image_size, rand_crop, mean=None, std=None, num_workers=6,
                        ...
                        offset_idx=0,
                        offset_label=0, sampler=None,drop_last=False, eval=False, autoaugment=False):
	'''
	...
	if isinstance(augment,bool):
		...
		if augment is not None:
			...
			# 	transform_list.append(transforms.RandomResizedCrop(size=rand_crop, scale=(0.875, 1.)))
			# elif image_size == 96:
			else:
				transform_list.append(transforms.Resize(image_size))
				if image_size == rand_crop:
					transform_list.append(transforms.RandomCrop(rand_crop, padding=4))
				else:
			...
			if autoaugment:
				pass# transform_list.append(AutoAugment())
			# transform_list.append(transforms.ColorJitter(.4, .4, .4))
	...
	loader = torch.utils.data.DataLoader(
		...
		shuffle=shuffle, num_workers=num_workers, pin_memory=False, drop_last=drop_last)
...
def get_classifier(classes, d, pretrained):
	...
	# elif d == "resnet":
	# 	cnn_ = resnet(num_classes=classes, depth=110)
	# elif d == "resnet50":
	# 	cnn_ = torchvision.models.resnet50()
	# 	num_ftrs = cnn_.fc.in_features
	# 	cnn_.fc = nn.Linear(num_ftrs, classes)
	# elif d == "resnet18":
	# 	cnn_ = torchvision.models.resnet18()
	# 	num_ftrs = cnn_.fc.in_features
	# 	cnn_.fc = nn.Linear(num_ftrs, classes)
	# elif d == "conv":
