...
from glo import nag_trainer
from glo.utils import NAGParams, OptParams, _load_lowshot_cifar #, get_cifar100_small
import socket
...
parser.add_argument('--noise_proj', action='store_true', help='is fewshot')
parser.add_argument('--d', type=str, choices=['vgg', 'wideresnet', 'none', 'conv'], default='none',
                    help='classifier name')
parser.add_argument('--data', type=str, choices=['cifar', 'cub', 'stl'], default='cifar', help='dataset')
...
if "samba" in socket.gethostname():  # or socket.gethostname() == "hachiman":
...
if data_name == 'cifar-100':
	...
	if args.fewshot:
		print("=> Fewshot")
		train_data, transductive_data = get_cifar100_small(cifar_dir_small, args.shot)
	else:
		train_data = torchvision.datasets.CIFAR100(root=data_dir, train=True, download=True)
		transductive_data = torchvision.datasets.CIFAR100(root=data_dir, train=False, download=True)
if args.data == 'cub':
	classes = 200
	batch_size = min(args.batch_size, 32)
	train_data = Cub2011(root=f"../../data/{args.data}", train=True)
	transductive_data = Cub2011(root=f"../../data/{args.data}", train=False)
if args.data == "stl":
	print("STL-10")
	classes = 10
	batch_size = min(args.batch_size, 32)
	train_data = torchvision.datasets.STL10(root=f"../../data/{args.data}", split='train', download=True)
	transductive_data = torchvision.datasets.STL10(root=f"../../data/{args.data}", split='unlabeled', download=True)
...
nag_params = NAGParams(nz=args.dim, force_l2=False, is_pixel=args.pixel, z_init=args.z_init,
                       is_classifier=is_classifier, disc_net=args.d, loss=args.loss, data_name=data_name,noise_proj=args.noise_proj,shot=args.shot)
nag_opt_params = OptParams(lr=args.lr, factor=args.factor, batch_size=batch_size, epochs=args.epoch,
                           decay_epochs=args.decay, decay_rate=0.5)
...
def get_run_name_from_args(args):
	...
	return f"{data_name}_{args.rn}{arg_str}_{args.z_init}_{nag_opt_params.decay_epochs}_{nag_opt_params.lr}_{nag_opt_params.factor}_{nag_opt_params.decay_rate}"
...
if args.tr:
	...
	nt = nag_trainer.NAGTrainer([train_data, [], transductive_data], nag_params, rn, resume=args.resume,
	                            num_classes=classes)
else:
	nt = nag_trainer.NAGTrainer([train_data, [], []], nag_params, rn, resume=args.resume, num_classes=classes)
