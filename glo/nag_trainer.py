...
from glo.model import Classifier
from glo.interpolate import save_inter_imgs
from glo import model
from glo.utils import *
from logger import Logger, savefig

image_path = '/cs/labs/daphna/daphna/data/imagenet/ILSVRC2012_img_train_preprocessed'
if socket.gethostname() == "kami":
	print(socket.gethostname())
	image_path = '/mnt/local/guy.hacohen/train'
...
class NAG():
	def __init__(self, nag_params, image_params, rn, resume, num_classes, resolution, datasets=None, is_ft=False):
		...
		if nag_params.data_name == 'cifar-100':
			self.aug_param = get_cifar_param()
		...
		if self.nag_params.is_classifier:
			if self.nag_params.is_pixel:
				if self.nag_params.disc_net == "vgg":
					...
					self.classifier = WideResNet(depth=16, num_classes=100, widen_factor=8, dropRate=0.3)
				elif self.nag_params.disc_net == "resnet":
					self.classifier = models.resnet50(pretrained=False, num_classes=self.num_classes)
				elif self.nag_params.disc_net == "conv":
					if self.nag_params.data_name == 'cifar-100':
						self.classifier = cnn(num_classes=self.num_classes, im_size=resolution)  # Pixel space
					elif self.nag_params.data_name == 'cub':
						self.classifier = model.DCGAN_D(num_classes=self.num_classes,
						                                image_size=resolution)  # Pixel space
					elif self.nag_params.data_name == 'stl':
						self.classifier = model.DCGAN_D_small(num_classes=self.num_classes,
						                                      image_size=resolution)  # Pixel space
			else:
	...
	def init_models_weights(self):
		...
		if self.nag_params.shot > 0:
			for k in self.netZ.label2idx.keys():
				assert (len(self.netZ.label2idx[k])) <= self.nag_params.shot, "len(self.netZ.label2idx[k])"
	...
	def train_long(self, train_data, test_data_fewshot, test_data_transductive, opt_params, resolution,
	               vis_epochs=100, save_ckpt=5):
		global epoch_loss, d_loss, epoch_loss_few_shot, d_loss_few_shot
		fewshot_data_loader = optimizerD = scheduler = transductive_data_loader = None
		...
		if len(test_data_fewshot) > 0:
			fewshot_data_loader = get_loader_with_idx(test_data_fewshot, batch_size,
		...
		if is_transductive:
			...
			transductive_data_loader = get_loader_with_idx(test_data_transductive, batch_size,
			                                               offset_idx=len(test_data_fewshot) + len(train_data),
			                                               offset_label=0,
		...
		while epoch < opt_params.epochs:
			...
			if self.nag_params.is_classifier:
				...
				# 	[{'params': self.classifier.parameters()}, {'params': self.netZ.parameters(), 'lr': 1e-5}],
				# 	lr=1e-3, betas=(0.5, 0.999), weight_decay=5e-4)

				optimizerD = optim.SGD([{'params': self.classifier.parameters()},
				                        {'params': self.netZ.parameters(), 'lr': 1e-5}],
			...
			if fewshot_data_loader is not None:
				_, _ = self.train_epoch(fewshot_data_loader, epoch=epoch,
				                        optim_Z=optimizerZ, optim_G=optimizerG,
				                        optim_D=optimizerD, scheduler=scheduler)
			if is_transductive:
			...
			# self.tensorboard._add_summary(epoch, {
			# 	"rec_loss": epoch_loss, "d_loss": d_loss, "rec_loss_few_shot": epoch_loss_few_shot,
			# 	"d_loss_few_shot": d_loss_few_shot
			# })
	...
	def train_epoch(self, data_loader, epoch, optim_Z, optim_G, optim_D, scheduler, is_transductive=False):
		'''
		...
		for i, batch in enumerate(data_loader):
			...
			epoch_loss = self.make_step(Ii, images, losses, optim_G, optim_Z, optim_D, d_loss=d_loss)
			if self.nag_params.is_classifier:
	...
	def make_step(self, Ii, images, losses, optimizerG, optimizerZ, optimizerD, d_loss=0):
		rec_loss = self.dist(2 * Ii - 1, 2 * images - 1)
		rec_loss = rec_loss.mean() + d_loss  # + slerp_loss
	...
	def get_classifier_loss(self, labels, zi, code):
		...
		if self.nag_params.loss == "cosine":
			...
			d_loss = self.d_criterion(outputs, onehot_labels.cuda(), self.y_target)
		else:
