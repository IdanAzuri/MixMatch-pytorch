...
from glo.utils import save_image_grid
...
def save_inter_imgs(netZ, netG, rn, name="", vis_n=10):
	...
	z_knn = [random.sample(netZ.label2idx[label], 2)[1] for label in target]
...
#
# for i in indices:
# 	y_1 = int(Yn[i, 0] * (RES - img_size))
# 	y_2 = int(Yn[i, 1] * (RES - img_size))
#
# 	# x = totensor(vec_mat[i].transpose((1, 2, 0)))
#
# 	can[y_1:y_1 + img_size, y_2:y_2 + img_size, :] = real_imgs[i]#[0].mul(255).clamp(0, 255).byte().permute(1, 2, 0).numpy()
# # plt.imsave(f'runs/ims_{run_name}/outfile_3000.jpg' , can)
# # print("save!")
# # plot_path(moves_knn, start_point_plt, targ_point_plt, title="RNN miniImagenet", more="")
# imageio.imwrite(f'runs/ims_{rn}/path_only_{RES}.jpg', can)
